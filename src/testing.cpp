...
{
	char* sep = strchr(D->word,'_');
	if (sep && IsDigit(sep[1])) Log(STDUSERLOG,"%s\r\n",D->word);
}
...
static void C_TopicDump(char* input)
{
	...
	for (unsigned int i = 1; i <= topicInfoPtr->numberOfTopics; ++i) 
	{
		...
		fprintf(out,"%s",DisplayTopicFlags(i));
		fprintf(out,"Bot: %s\r\n",topicInfoPtr->topicRestrictionMap[i] ? topicInfoPtr->topicRestrictionMap[i] : (char*)"all");
...
static void C_TopicInfo(char* input)
{
	...
	for (unsigned int topicid = 1; topicid <= topicInfoPtr->numberOfTopics; ++topicid) 
	{
		...
		if (all) 
		{
			...
			Log(STDUSERLOG,"Bot: %s\r\n",topicInfoPtr->topicRestrictionMap[topicid] ? topicInfoPtr->topicRestrictionMap[topicid] : (char*)"all");
			if (topicInfoPtr->topicLastGambittedMap[topicid] == 0 && topicInfoPtr->topicLastResponderedMap[topicid] == 0 && topicInfoPtr->topicLastRejoinderedMap[topicid] == 0) Log(STDUSERLOG,"Seen: never visited");
			else Log(STDUSERLOG,"Seen: last gambit %d   last rejoinder %d  lastresponder\r\n", topicInfoPtr->topicLastGambittedMap[topicid],topicInfoPtr->topicLastResponderedMap[topicid],topicInfoPtr->topicLastRejoinderedMap[topicid]);
		}
...
{//  actual command names must be lower case 
	...
	{":topicdump",C_TopicDump,"dump keywords as a space separated string, followed by rules, one per line til null char"},
