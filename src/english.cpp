...
{ // this is not allowed to write properties/systemflags/internalbits if the word is preexisting
	...
	if (start != at && tokenControl & STRICT_CASING) {;} // believe all upper case items not at sentence start when using strict casing
	...
	{
		char* verb =  English_GetInfinitive(original,true); 
	...
	if (!properties && hyphen)
	{
		*hyphen = 0;
		WORDP X = FindWord(original,LOWERCASE_LOOKUP);
		WORDP Y = FindWord(hyphen+1,LOWERCASE_LOOKUP);
		*hyphen = '-';
...
static char* InferAdjective(char* original, unsigned int len) // might it be adjective based on suffix? If so, enter into dictionary
{
	...
	if (flags) 
	{
		return StoreWord(word,flags)->word;
	}
