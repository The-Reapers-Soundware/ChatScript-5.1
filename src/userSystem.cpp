...
{ 
	...
	if ((!server || serverRetryOK) && !documentMode && !callback)  CopyFile2File("TMP/backup.bin",userDataBase,false);	// backup for debugging BUT NOT if callback of some kind...
#endif
...
void ReadNewUser()
{
	...
	*buffer = 0;
	...
	value = GetUserVariable("$response");
	ReadInt64(value,v);
	responseControl = (*value) ? (unsigned int)v : ALL_RESPONSES;
